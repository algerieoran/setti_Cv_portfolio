...
if(!empty($_POST)) {  //SI LE FORMULAIRE est sumis
    ...
    if (empty($contenu)) {  // s'il n'y a pas d'erreur sur le formulaire
        $user = executeRequete("SELECT * FROM t_utilisateurs WHERE pseudo = :pseudo AND mdp = :mdp", array(':pseudo' => $_POST['pseudo'], ':mdp' => $_POST['mdp']));

         if ($user->rowCount() > 0) {  // si la requete retourne est superieur à 0, alors le login et le mdp existent ensemble en BDD 
            // on crée une session avec les informations du membre :
                $informations = $user->fetch(PDO::FETCH_ASSOC);  // on fait un fetch pour transformer l'objet $membre en un array associatif qui contient en indices le nom de tous les champs de la requête
                debug($informations);

                $_SESSION ['t_utilisateurs'] = $informations;   // nous créons une session avec les infod du membre qui proviennent de la BDD.
